...
We will also evaluate this using cross-validation because we'll need that later when we stack models. I'm using the same seed I used in the previous tutorial when we fit a linear regression because I want the same folds. 
...
We will create one use one more model type - K-nearest neighbors. We can use the same recipes and step as in the lasso model. So, we define the model, leaving the number of neighbors (k) to be tuned; create the workflow; and tune the model (install `kknn` package first - don't need to load):
