...
2. In this example, we will compute some summary statistics about the `flights` dataset. We will find the minimum and maximum years in the dataset, the total number of flights, and the number of distinct carriers. Just like when we did this with a SQL query, this does take a minute to run. (**NOTE**: This chunk no longer works! There are [workarounds](https://community.rstudio.com/t/dbplyr-summarize-without-a-group-by/116466), but it does not seem necessary to discuss at this point in time.)
