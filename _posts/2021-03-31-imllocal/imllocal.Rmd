...
Let's look at an example and then discuss the output. I `set.seed()` for reproducibility of the random perturbations. Like I mentioned above, we need to eliminate the outcome variable from the observation of interest. I also eliminated variables 

```{r}
# Figure out why I need this code - order of libraries?
# Taken from: https://rdrr.io/cran/DALEXtra/src/R/predict_surrogate.R

# model_type(rf_explain) ... gives error ... why??
...
# may have to change this to 'classification' for classification
model_type.dalex_explainer <- function(x, ...) 'regression'
...
```{r}
# This code initially worked without the code chunk above - why not now?
